...
def get_python_docstring(file_path):
    ...
    return docstring.strip()[:-3] if docstring else "No docstring available"
...
def generate_tree(directory, indent=0, tree_str="", last_item=False):
    ...
    if not os.path.exists(directory):  # check directory
        return f"Error: Directory {directory} does not exist."

    items = sorted(os.listdir(directory))
    for i, item in enumerate(items): # generate tree
        ...
        if os.path.isdir(item_path):
            ...
            elif item == "keyphrase_extraction": tree_str += f"{prefix}{item}/  # Contain Keyphrase Extraction functionality temporary files.\n"
            ...
            if item.endswith(".py"): 
                tree_str += f"{prefix}{item}  # {get_python_docstring(item_path)}\n"
            elif item.endswith(".css"): 
                tree_str += f"{prefix}{item}  # {get_css_comment(item_path)}\n"
            elif item.endswith(".js"): 
                tree_str += f"{prefix}{item}  # {get_js_comment(item_path)}\n"
            elif item.endswith(".html"): 
                tree_str += f"{prefix}{item}  # {get_html_comment(item_path)}\n"
            else: 
...
def generate_readme(path):
    tree_output = generate_tree(path) # generating tree
    tree_output = "```bash\n" + tree_output + "```"  # markdown
    with open("README.md", "w", encoding="utf-8") as f:
        f.write(tree_output) # printing readme file
