...
class SeasonalityPredictor:
    ...
    def __init__(self, config: PlotConfig, periodicity: int, nlags: int) -> None:
    ...
    def save_predictions(
        ...
        acf_values = {'LAG': range(1, len(acf_values) + 1), 'CORRELATION (%)': acf_values}
        pacf_values = {'LAG': range(1, len(pacf_values) + 1), 'CORRELATION (%)': pacf_values}

        with pd.ExcelWriter(os.path.join(self.config.save_dir, file_name), engine='openpyxl') as w:
            pd.DataFrame(forecast).to_excel(w, sheet_name='Forecast', index=False)
            pd.DataFrame(acf_values).to_excel(w, sheet_name='Autocorrelation', index=False)
            pd.DataFrame(pacf_values).to_excel(w, sheet_name='Parcial Autocorrelation', index=False)

class SeasonalityPlotter:

    ac_title = "Autocorrelation"
    pac_title = "Partial Autocorrelation"

    def __init__(self, config: PlotConfig, nlags: int) -> None:
        self.config = config
        self.nlags = nlags
    ...
    def plot_decomposition(self, serie: pd.Series, periodicity: int) -> None:
        decomposed_values = seasonal_decompose(serie, model='additive', period=periodicity)

        fig, axes = plt.subplots(4, 1, figsize=(12, 9), sharex=True)
        decomposed_values.observed.plot(ax=axes[0])
        axes[0].set_title('Original', fontsize=self.config.plot_title_fontsize)

        decomposed_values.trend.plot(ax=axes[1])
        axes[1].set_title('Trend', fontsize=self.config.plot_title_fontsize)

        decomposed_values.seasonal.plot(ax=axes[2])
        axes[2].set_title('Seasonality', fontsize=self.config.plot_title_fontsize)

        decomposed_values.resid.plot(ax=axes[3])
        axes[3].set_title('Residuals', fontsize=self.config.plot_title_fontsize)

        self._save_figure(fig, 'data_decomposition.png')

    def plot_autocorrelation(self, serie: pd.Series) -> None:

        fig, axes = plt.subplots(1, 2, figsize=(14, 4))

        plot_acf(serie, lags=self.nlags, ax=axes[0])
        axes[0].set_title(self.ac_title, fontsize=self.config.plot_title_fontsize)
        axes[0].set_xlabel("Periods", fontsize=self.config.plot_title_fontsize)

        plot_pacf(serie, lags=self.nlags, ax=axes[1], method='ols')
        axes[1].set_title(self.pac_title, fontsize=self.config.plot_title_fontsize)
        axes[1].set_xlabel("Periods", fontsize=self.config.plot_title_fontsize)

        self._save_figure(fig, 'autocorrelation_data.png')
    ...
    def generate_all_plots(
        ...
        self.plot_decomposition(serie, periodicity)
        self.plot_autocorrelation(serie)

class SeasonalityModule:

    def __init__(self, file: str, periodicity: int, nlags: int, plot_config: PlotConfig | None = None) -> None:
        self.serie = None
        self.file = file
        self.periodicity = periodicity
        self.nlags = nlags
        self.config = plot_config or PlotConfig()
        self.plotter = SeasonalityPlotter(self.config, self.nlags)
        self.predictor = SeasonalityPredictor(self.config, self.periodicity, self.nlags)
